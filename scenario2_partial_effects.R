...
sam_pe <- parallel::mclapply(sam_par, sam_eff, mc.cores = 10)
...
rm(list = c("sam_par", "sam_pe", "sam_eff", "sam_xy"))

#---------------------------------------------------------------------------

######################## BORAL #############################

load("sc2_boral.RData")
...
# parallel processing of 100 xx and yy values
hmsc_pe <- parallel::mclapply(hm_fit, hmsc_eff, mc.cores = 10)
...
rm(list = c("hm_fit", "hmsc_pe", "hmsc_eff", "hmsc_xy"))
...
# parallel processing of 100 xx and yy values
mistnet_pe <- mclapply(mn_fit, mistnet_eff, mc.cores = 10)
...
rm(list = c("mn_fit", "mistnet_pe", "mistnet_eff", "mn_xy", "i"))
...
# save(boral_pp, brt_pp, glm_pp, hmsc_pp, mistnet_pp, raw_pp, sam_pp, file = "sc2_partial_effects.RData")
