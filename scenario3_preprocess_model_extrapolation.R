...
############################ Model Extrapolations (except HMSC and BORAL) for Scenario 3 only ###############
######################### Raw ###########################

# load extrapolation data
load("extrapolation_data.RData")
load("sc3_simulations.RData")
...
rawp_full_fun <- function(data){
  ...
  raw <- as.matrix(data.frame(intercept = c(0,0,0,0),
                              sst = c(0, 1.5, -1.5, 1),
                              sst2 = c(0, 0, 0, -1),
                              dsStart = c(-0.25, 1.5, -0.05, 0.25),
                              chla = c(0, 0, 0, 0)))
  
  spp_arc <- simulation3[["info"]][[1]][["SAMs"]]
  
  raw <- raw[spp_arc, , drop = FALSE]
  rownames(raw) <- simulation3[["info"]][[1]][["names"]][1:18]
  raw[,1] <- simulation3[["info"]][[1]][["alpha"]]
...
save(raw_ext, file = "sc3_raw_extrapolations.RData")
...
save(sam_final, sam_full, sam_initial, file = "sc3_sam_extrapolations.RData")
...
save(sam_ext, file = "sc3_sam_extrapolations_mean.RData")

############################ MISTNET ###################################

load("sc3_mistnet.RData")
...
save(mistnet_final, mistnet_full, mistnet_initial, file = "sc3_mistnet_extrapolations.RData")
...
save(mistnet_ext, file = "sc3_mistnet_extrapolations_mean.RData")
...
save(glm_final, glm_full, glm_initial, file = "sc3_glm_extrapolations.RData")
...
save(glm_ext, file = "sc3_glm_extrapolations_mean.RData")
...
save(brt_final, brt_full, brt_initial, file = "sc3_brt_extrapolations.RData")
...
save(brt_ext, file = "sc3_brt_extrapolations_mean.RData")
