...
for(i in 1:length(hmsc_par)){
  ...
  fitSepTF <- hmsc_par[[i]]$hmsc_fit

  #assign species names
  sp.v <- fitSepTF$spNames
  ...
  foreach(l=1:length(cell.sel.v)) %dopar%{
    ...
    ## setup prediction - pa
    Gradient.pa = prepareGradient(fitSepTF, XDataNew = XData.grid.loop, sDataNew = list(sample = xy.grid.loop))
    Gradient.pa$studyDesignNew <- data.frame(sample = as.factor(1:length(xy.grid.loop$x)))

    sdes <- data.frame(sample = as.factor(1:length(xy.grid.loop$x)))
    predY.loop.pa <- Hmsc:::predict.Hmsc(fitSepTF, XData = Gradient.pa$XDataNew, studyDesign = sdes, ranLevels =  Gradient.pa$rLNew,
                                         expected = TRUE, predictEtaMean = TRUE)

    rm(Gradient.pa)
    predY.loop.pa.array = simplify2array(predY.loop.pa)

    # Get posterior mean and std.error
...
for (i in 1:length(hmsc_par)) {
  ...
  num_cores <- parallel::detectCores() # Use all but one core
...
for(i in 1:length(hmsc_par)){
  ...
  foreach(l=1:length(cell.sel.v)) %dopar%{ 
    ...
    rm(predY.loop.pa, predY.pa.mean)#, predY.pa.up, predY.pa.low)
  }
  computational.time = proc.time() - ptm
  parallel::stopCluster(cl = c1)
